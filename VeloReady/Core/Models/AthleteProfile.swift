...
class AthleteProfileManager: ObservableObject {
    ...
    init() {
        ...
        if let data = UserDefaults.standard.data(forKey: userDefaultsKey),
           let decoded = try? JSONDecoder().decode(AthleteProfile.self, from: data) {
            ...
            print("📊 Loaded athlete profile from cache:")
            print("   Adaptive FTP: \(decoded.ftp ?? 0)W (\(decoded.ftpSource.rawValue))")
            print("   Max HR: \(decoded.maxHR ?? 0)bpm (\(decoded.hrZonesSource.rawValue))")
            if let wPrime = decoded.wPrime {
                print("   W': \(Int(wPrime))J")
            }
            if let vo2max = decoded.vo2maxEstimate {
                print("   VO2max: \(Int(vo2max)) ml/kg/min")
            }
            if let quality = decoded.dataQuality {
                print("   Data Quality: \(Int(quality.confidenceScore * 100))%")
            }
        } else {
            self.profile = AthleteProfile()
            print("📊 Created new athlete profile")
        }
    }
    ...
    func computeFromActivities(_ activities: [IntervalsActivity]) {
        print("📊 ========== COMPUTING ADAPTIVE ZONES FROM PERFORMANCE DATA ==========")
        print("📊 Using modern sports science algorithms (CP model, power distribution, HR analysis)")
        ...
        print("📊 Found \(recentActivities.count) activities in last 120 days")
        print("📊 Ignoring hardcoded Intervals.icu zones - computing from actual performance data")
        ...
        if profile.ftpSource != .manual {
            ...
            print("📊 Skipping FTP computation - user has manual override (FTP: \(profile.ftp ?? 0)W)")
        }
        
        if profile.hrZonesSource != .manual {
            computeHRZonesFromPerformanceData(recentActivities)
        } else {
            print("📊 Skipping HR zones computation - user has manual override (Max HR: \(profile.maxHR ?? 0)bpm)")
        }
    ...
    private func computeFTPFromPerformanceData(_ activities: [IntervalsActivity]) {
        print("📊 ========== FTP COMPUTATION (ENHANCED v2) ==========")
        print("📊 Target: ±2-5% accuracy with confidence-based buffer")
        ...
        print("📊")
        print("📊 STAGE 1: Building Power-Duration Curve")
        print("📊 Analyzing \(activities.count) activities...")
        ...
        for (index, activity) in activities.enumerated() {
            ...
            if np > 0 {
                print("📊   Activity \(index + 1): \(activity.name ?? "Unnamed") - NP: \(Int(np))W, Duration: \(Int(duration/60))min")
                ...
                if duration >= 10800 { // 3+ hours
                    ...
                    if duration >= 18000 { // 5+ hours
                        ...
                        print("📊     ✓ Ultra-endurance (5+ hours): \(Int(np))W → Estimated 60-min power: \(Int(np * boost))W")
                    } else if duration >= 14400 { // 4-5 hours
                        boost = 1.10 // NP is ~91% of FTP
                        print("📊     ✓ Ultra-endurance (4-5 hours): \(Int(np))W → Estimated 60-min power: \(Int(np * boost))W")
                    } else { // 3-4 hours
                        boost = 1.07 // NP is ~93% of FTP
                        print("📊     ✓ Ultra-endurance (3-4 hours): \(Int(np))W → Estimated 60-min power: \(Int(np * boost))W")
                    }
                    ...
                    if np > best60min {
                        best60min = np
                        print("📊     ✓ New best 60-min power: \(Int(np))W")
                    }
                }
                ...
                if duration >= 1200 { // 20+ min
                    if np > best20min {
                        best20min = np
                        print("📊     ✓ New best 20-min power: \(Int(np))W")
                    }
                }
                if duration >= 300 { // 5+ min
                    if np > best5min {
                        best5min = np
                        print("📊     ✓ New best 5-min power: \(Int(np))W")
                    }
                }
            }
        }
        ...
        print("📊")
        print("📊 Power-Duration Curve Results:")
        ...
        print("📊   Max NP: \(Int(maxNP))W")
        
        guard maxNP > 0 else {
            print("📊 ❌ No power data available - cannot compute FTP")
            return
        }
        
        // STAGE 2: Compute FTP candidates with weighting
        print("📊")
        print("📊 STAGE 2: Computing FTP Candidates")
        ...
        if best60min > 0 {
            ...
            print("📊   Method 1 (60-min): \(Int(best60min))W × 0.99 = \(Int(ftp))W (weight: \(String(format: "%.1f", weight)))")
        }
        ...
        if best20min > 0 {
            ...
            print("📊   Method 2 (20-min): \(Int(best20min))W × 0.95 = \(Int(ftp))W (weight: 0.9)")
        }
        ...
        if best5min > 0 {
            ...
            print("📊   Method 3 (5-min): \(Int(best5min))W × 0.87 = \(Int(ftp))W (weight: 0.6)")
        }
        
        guard !candidates.isEmpty else {
            print("📊 ❌ No valid candidates - cannot compute FTP")
            return
        }
        ...
        print("📊   Weighted FTP: \(Int(weightedFTP))W")
        
        // STAGE 3: Calculate confidence and apply buffer
        print("📊")
        print("📊 STAGE 3: Confidence Analysis & Buffer")
        
        let confidence = min(totalWeight / 2.5, 1.0)
        print("📊   Confidence score: \(String(format: "%.2f", confidence)) (0.0-1.0 scale)")
        ...
        if confidence >= 0.9 {
            bufferPercent = 1.02
            print("📊   Confidence: HIGH ≥0.9")
            print("📊   Applying +2% buffer (conservative estimate)")
        } else if confidence >= 0.7 {
            bufferPercent = 1.03
            print("📊   Confidence: MEDIUM ≥0.7")
            print("📊   Applying +3% buffer")
        } else {
            bufferPercent = 1.05
            print("📊   Confidence: LOW <0.7")
            print("📊   Applying +5% buffer")
        }
        
        bufferedFTP = weightedFTP * bufferPercent
        print("📊   Buffered FTP: \(Int(weightedFTP))W × \(String(format: "%.2f", bufferPercent)) = \(Int(bufferedFTP))W")
        
        // STAGE 4: Validation & Bounds
        print("📊")
        print("📊 STAGE 4: Validation & Bounds Check")
        
        let lowerBound = maxNP * 0.85
        let upperBound = maxNP * 1.05
        
        print("📊   Max NP: \(Int(maxNP))W")
        print("📊   Lower bound (85% of max NP): \(Int(lowerBound))W")
        print("📊   Upper bound (105% of max NP): \(Int(upperBound))W")
        print("📊   Buffered FTP: \(Int(bufferedFTP))W")
        
        var computedFTP = bufferedFTP
        
        if bufferedFTP < lowerBound {
            computedFTP = lowerBound
            print("📊   ⚠️  Below lower bound! Adjusted to: \(Int(computedFTP))W")
        } else if bufferedFTP > upperBound {
            computedFTP = upperBound
            print("📊   ⚠️  Above upper bound! Adjusted to: \(Int(computedFTP))W")
        } else {
        ...
        if let previousFTP = profile.ftp, previousFTP > 0 {
            ...
            if ultraEnduranceBoost != nil && confidence >= 0.9 {
                ...
                print("📊   Using balanced smoothing (ultra-endurance data)")
            } else if confidence >= 0.9 {
                smoothingRatio = (0.6, 0.4) // Less conservative for high confidence
                print("📊   Using moderate smoothing (high confidence)")
            } else {
                smoothingRatio = (0.7, 0.3) // Conservative for low confidence
                print("📊   Using conservative smoothing (standard)")
            }
            ...
            print("📊   Previous FTP: \(Int(previousFTP))W")
            print("📊   Raw computed: \(Int(computedFTP))W")
            print("📊   Smoothed FTP: \(Int(smoothedFTP))W (change: \(String(format: "%.1f", change))%)")
        ...
        print("📊 ✅ Adaptive FTP: \(Int(computedFTP))W")
        print("📊 Adaptive Power Zones: \(profile.powerZones!.map { Int($0) })")
        print("📊 Data Quality: \(Int(profile.dataQuality!.confidenceScore * 100))% confidence from \(profile.dataQuality!.sampleSize) activities")
    }
    ...
    private func buildPowerDurationProfile(activities: [IntervalsActivity]) -> PowerDurationProfile {
        ...
        if !ultraEndurance.isEmpty {
            ...
            print("📊   Ultra-endurance ride found: \(Int(maxUltraNP))W for \(Int(maxUltraDuration/3600))h → Est. 60-min: \(Int(profile.power60min!))W")
        } else {
    ...
    private func computeHRZonesFromPerformanceData(_ activities: [IntervalsActivity]) {
        print("📊 === HR ZONES COMPUTATION (Lactate Threshold Detection) ===")
        ...
        guard !maxHRValues.isEmpty else {
            print("📊 ❌ No HR data available - cannot compute HR zones")
            return
        }
        ...
        print("📊 Max HR Analysis:")
        print("📊   Observed max: \(Int(observedMaxHR))bpm")
        print("📊   Top 5% avg: \(Int(avgTop5HR))bpm")
        print("📊   Computed max (with 2% buffer): \(Int(computedMaxHR))bpm")
        ...
        if !sustainedMaxHREfforts.isEmpty {
            ...
            lthrEstimate = median
            print("📊 LTHR detected from \(sustainedMaxHREfforts.count) sustained efforts: \(Int(lthrEstimate!))bpm (median max HR)")
            print("📊   Range: \(Int(sorted.first!))- \(Int(sorted.last!))bpm")
        } else {
            ...
            if !intensityBasedEfforts.isEmpty {
                ...
                if !trimmed.isEmpty {
                    ...
                    print("📊 LTHR estimated from \(intensityBasedEfforts.count) high-intensity efforts: \(Int(lthrEstimate!))bpm")
                    print("📊   Range: \(Int(sorted.first!))- \(Int(sorted.last!))bpm (trimmed mean)")
                }
            } else {
                print("📊 ⚠️ No suitable threshold efforts found for LTHR estimation")
            }
        }
        ...
        if let previousMaxHR = profile.maxHR, previousMaxHR > 0 {
            ...
            print("📊   Previous Max HR: \(Int(previousMaxHR))bpm")
            print("📊   Raw computed: \(Int(computedMaxHR))bpm")
            print("📊   Smoothed Max HR: \(Int(smoothedMaxHR))bpm (change: \(String(format: "%.1f", change))%)")
        ...
        if let lthr = lthrEstimate {
            ...
            if lthrPercentage >= 0.82 && lthrPercentage <= 0.93 {
                ...
                print("📊 ✅ HR Zones (Adaptive - LTHR anchored): \(profile.hrZones!.map { Int($0) })")
                print("📊 LTHR: \(Int(lthr))bpm (\(Int(lthrPercentage * 100))% of max) - Valid range ✓")
            } else {
                profile.hrZones = AthleteProfileManager.generateHRZones(maxHR: computedMaxHR)
                print("📊 ✅ HR Zones (Coggan): \(profile.hrZones!.map { Int($0) })")
                print("📊 ⚠️ LTHR: \(Int(lthr))bpm (\(Int(lthrPercentage * 100))% of max) - Outside valid range (82-93%), using Coggan zones")
            }
        } else {
            profile.hrZones = AthleteProfileManager.generateHRZones(maxHR: computedMaxHR)
            print("📊 ✅ HR Zones (Coggan): \(profile.hrZones!.map { Int($0) })")
            print("📊 ⚠️ No LTHR detected - using Coggan zones")
        }
        
        print("📊 Max HR: \(Int(computedMaxHR))bpm")
        if let lthr = lthrEstimate {
            print("📊 LTHR: \(Int(lthr))bpm (\(Int((lthr / computedMaxHR) * 100))% of max)")
        }
    }
    ...
    private func generateAdaptiveHRZones(maxHR: Double, lthr: Double) -> [Double] {
        ...
        print("📊 Adaptive zone computation: LTHR is \(Int(lthrPercentage * 100))% of max HR")
    ...
    private func updateAuxiliaryMetrics(_ activities: [IntervalsActivity]) {
        ...
        if let restingHR = activities.compactMap({ $0.icuRestingHr }).first {
            ...
            print("📊 Updated Resting HR: \(Int(restingHR))bpm")
        }
    ...
    func setManualFTP(_ ftp: Double, zones: [Double]?) {
        print("📊 Manual FTP override: \(Int(ftp))W")
        profile.ftp = ftp
    ...
    func setManualMaxHR(_ maxHR: Double, zones: [Double]?) {
        print("📊 Manual Max HR override: \(Int(maxHR))bpm")
        profile.maxHR = maxHR
    ...
    func resetToComputed() {
        print("📊 Resetting to computed values")
        profile.ftpSource = .computed
        profile.hrZonesSource = .computed
        profile.lastUpdated = Date()
        save()
    ...
    func save() {
        if let encoded = try? JSONEncoder().encode(profile) {
            ...
            print("📊 Saved athlete profile to cache")
        }
    }
