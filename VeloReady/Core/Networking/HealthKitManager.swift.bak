...
class HealthKitManager: ObservableObject {
    ...
    func requestAuthorization() async {
        ...
        do {
            ...
            print("📋 Requesting permissions for: \(readTypes.map { $0.identifier })")
            try await healthStore.requestAuthorization(toShare: [], read: readTypes)
            print("✅ Authorization sheet completed")
    ...
    func requestWorkoutPermissions() async {
        ...
        do {
            ...
            print("✅ Workout authorization sheet completed")
            ...
            print("❌ Workout authorization error: \(error.localizedDescription)")
        }
    }
    ...
    func requestWorkoutRoutePermissions() async {
        ...
        do {
            print("🔐 Requesting Workout + Route permissions...")
            let workoutType = HKObjectType.workoutType()
            let routeType = HKSeriesType.workoutRoute()
            
            // Must request both together
            try await healthStore.requestAuthorization(toShare: [], read: [workoutType, routeType])
            print("✅ Workout + Route authorization sheet completed")
            
            // Check status
            let workoutStatus = healthStore.authorizationStatus(for: workoutType)
            let routeStatus = healthStore.authorizationStatus(for: routeType)
            print("📊 Workout permission status: \(workoutStatus.rawValue)")
            print("📊 Route permission status: \(routeStatus.rawValue)")
            
        } catch {
            print("❌ Workout Route authorization error: \(error.localizedDescription)")
        }
    }
    ...
    func getAuthorizationStatus(for type: HKObjectType) -> HKAuthorizationStatus {
        ...
        if status.rawValue == 1 && status == .sharingDenied {
            print("🔧 Fixing authorization status: rawValue 1 treated as authorized for \(type.identifier)")
    ...
    func checkAuthorizationStatusFast() async {
        print("⚡ Fast HealthKit authorization check")
        ...
        if hrvStatus.rawValue == 1 && hrvStatus == .sharingDenied {
            print("🔧 Fast check: rawValue 1 treated as authorized for \(hrvType.identifier)")
        ...
        print("⚡ Fast HealthKit check completed: \(await authorizationState)")
    }
    ...
    private func updateAuthorizationState(authorizedCount: Int, deniedCount: Int, notDeterminedCount: Int) {
        ...
        if authorizedCount == readTypes.count {
            ...
            isAuthorized = false
            print("⚠️ HealthKit denied - critical permissions missing (\(deniedCount) denied, \(authorizedCount) authorized)")
            ...
            isAuthorized = true
            print("✅ HealthKit partially authorized (\(authorizedCount)/\(readTypes.count))")
        } else {
        ...
        return await withCheckedContinuation { (continuation: CheckedContinuation<HealthKitSleepData?, Never>) in
            let query = HKSampleQuery(
                ...
                guard error == nil, let sleepSamples = samples as? [HKCategorySample], !sleepSamples.isEmpty else {
                    print("❌ No HealthKit sleep samples found: error=\(error?.localizedDescription ?? "none"), samplesCount=\(samples?.count ?? 0)")
                ...
                guard let mostRecentSession = sleepSessions.last else {
                    print("❌ No valid sleep sessions found")
                    continuation.resume(returning: nil)
                    return
                }
                
                print("✅ Using most recent sleep session (\(mostRecentSession.count) samples)")
                ...
                print("💤 Sleep: \(String(format: "%.1f", totalSleep/3600))h (Deep: \(String(format: "%.1f", deepSleep/3600))h, REM: \(String(format: "%.1f", remSleep/3600))h, Core: \(String(format: "%.1f", coreSleep/3600))h)")
                ...
                if actualSleepDuration > 8*3600 && actualTimeInBed < actualSleepDuration {
                    print("⚠️ POTENTIAL VALUE SWAP DETECTED: Swapping sleepDuration and timeInBed")
                    correctedSleepDuration = actualTimeInBed
                    correctedTimeInBed = actualSleepDuration
                    print("   Corrected Sleep Duration: \(String(format: "%.2f", correctedSleepDuration/3600)) hrs")
                    print("   Corrected Time in Bed: \(String(format: "%.2f", correctedTimeInBed/3600)) hrs")
                }
    ...
    func fetchHistoricalSleepData(days: Int = 7) async -> [(bedtime: Date?, wakeTime: Date?)] {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                guard error == nil, let sleepSamples = samples as? [HKCategorySample], !sleepSamples.isEmpty else {
                    print("❌ No historical sleep data found: error=\(error?.localizedDescription ?? "none"), samplesCount=\(samples?.count ?? 0)")
                    continuation.resume(returning: [])
                    return
                }
                
                print("🔍 HISTORICAL SLEEP ANALYSIS:")
                print("📊 Found \(sleepSamples.count) historical sleep samples over \(days) days")
                ...
                print("   Found \(allSleepSessions.count) total sleep sessions across \(days) days")
                ...
                for (index, session) in allSleepSessions.enumerated() {
                    ...
                    print("     Session \(index + 1): bedtime=\(earliestBedtime?.description ?? "nil"), wake=\(latestWakeTime?.description ?? "nil")")
                }
    ...
    func fetchOvernightHRVData(bedtime: Date? = nil, wakeTime: Date? = nil) async -> (sample: HKQuantitySample?, value: Double?) {
        ...
        if let bedtime = bedtime, let wakeTime = wakeTime {
            ...
            print("🍷 Using ACTUAL sleep session times for overnight HRV")
        } else {
            ...
            print("🍷 Using ESTIMATED sleep window for overnight HRV (no sleep session data)")
        }
        
        print("🍷 Fetching overnight HRV from \(overnightStart) to \(overnightEnd)")
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                guard let samples = samples as? [HKQuantitySample], !samples.isEmpty else {
                    print("🍷 ⚠️ No overnight HRV samples found - alcohol detection will be limited")
                ...
                print("🍷 Found \(samples.count) overnight HRV samples")
                print("🍷 Average overnight HRV: \(String(format: "%.2f", average)) ms (range: \(String(format: "%.2f", values.min() ?? 0))-\(String(format: "%.2f", values.max() ?? 0)))")
    ...
    private func fetchSum(for type: HKQuantityType, predicate: NSPredicate, unit: HKUnit) async -> Double {
        return await withCheckedContinuation { continuation in
            let query = HKStatisticsQuery(
                ...
                print("✅ Fetched \(type.identifier): \(value)")
    ...
    func fetchVO2MaxData(from startDate: Date, to endDate: Date) async throws -> [HKQuantitySample] {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                if let error = error {
                    print("❌ Error fetching VO₂ Max data: \(error.localizedDescription)")
                    continuation.resume(returning: [])
                    return
                }
                
                let vo2Samples = samples as? [HKQuantitySample] ?? []
                print("📊 Fetched \(vo2Samples.count) VO₂ Max samples")
    ...
    func fetchOxygenSaturationData(from startDate: Date, to endDate: Date) async throws -> [HKQuantitySample] {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                print("📊 Fetched \(spo2Samples.count) SpO2 samples")
    ...
    func fetchBodyTemperatureData(from startDate: Date, to endDate: Date) async throws -> [HKQuantitySample] {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                if let error = error {
                    print("❌ Error fetching body temperature data: \(error.localizedDescription)")
                    continuation.resume(returning: [])
                    return
                }
                
                let tempSamples = samples as? [HKQuantitySample] ?? []
                print("📊 Fetched \(tempSamples.count) body temperature samples")
    ...
    func fetchRecentWorkouts(limit: Int = 50, daysBack: Int = 30) async -> [HKWorkout] {
        ...
        print("🔍 Fetching workouts from \(startDate) to \(now)")
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                print("✅ Fetched \(workouts.count) strength/walking workouts from Apple Health")
